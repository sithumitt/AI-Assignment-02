...
# Dynamically locate the models folder relative to this script or root
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
POSSIBLE_MODEL_DIRS = [
    os.path.join(BASE_DIR, "models"),
    os.path.join(BASE_DIR, "..", "models"),
    os.path.join(os.getcwd(), "models"),
    os.path.join(os.getcwd(), "AI-Assignment", "models")
]
...
def load_artifacts():
    if not MODELS_DIR:
        raise FileNotFoundError("Could not find 'models/best_model.pkl' in any expected directory.")
    
    model = joblib.load(os.path.join(MODELS_DIR, "best_model.pkl"))
    scaler = joblib.load(os.path.join(MODELS_DIR, "scaler.pkl"))
    columns = joblib.load(os.path.join(MODELS_DIR, "model_columns.pkl"))
...
try:
    ...
    st.info("Ensure best_model.pkl, scaler.pkl, and model_columns.pkl are committed in the 'models/' directory.")
    ready = False
...
if ready:
    with st.form("risk_assessment_form"):
        # Section 1: Primary Biomarkers (High Priority)
        ...
        # Section 2: Medical History (Medium Priority)
